...
public class HeatExchangerCoolantPortScreen extends ScreenBase<HeatExchangerCoolantPortContainer> {
    ...
    public void render(@Nonnull PoseStack mStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (heatExchangerCoolantPortState.condenser) {
            ...
            this.getFont().draw(mStack, new TranslatableComponent("screen.biggerreactors.heat_exchanger_coolant_port.channel_type.condenser").getString(), this.getGuiLeft() + 8, this.getGuiTop() + 37, 4210752);
        } else {
            // Text for when connected to an evaporator.
            this.getFont().draw(mStack, new TranslatableComponent("screen.biggerreactors.heat_exchanger_coolant_port.channel_type.evaporator").getString(), this.getGuiLeft() + 8, this.getGuiTop() + 37, 4210752);
        }

        // Render text for input/output direction:
        if (heatExchangerCoolantPortState.direction) {
            // Text for an inlet:
            this.getFont().draw(mStack, new TranslatableComponent("screen.biggerreactors.heat_exchanger_coolant_port.direction_toggle.input").getString(), this.getGuiLeft() + 42, this.getGuiTop() + 22, 4210752);
        } else {
            // Text for an outlet:
            this.getFont().draw(mStack, new TranslatableComponent("screen.biggerreactors.heat_exchanger_coolant_port.direction_toggle.output").getString(), this.getGuiLeft() + 42, this.getGuiTop() + 22, 4210752);
        }
    }
}
