...
public class MultithreadedReactorSimulation implements IReactorSimulation {
        ...
        for (int j = 0; j < rays.size(); j++) {
            ...
            for (int k = 0; k < raySteps.size(); k++) {
                ...
                if (properties != null) {
                    ...
                    results.caseRFAdded += properties.heatEfficiency() * radiationAbsorbed * Config.Reactor.Modern.FEPerRadiationUnit;
                } else {
                    // its a fuel rod!
                    ...
                    results.fuelRFAdded += radiationAbsorbed * Config.Reactor.Modern.FEPerRadiationUnit;
    ...
    private void radiate() {
        ...
        final double rawIntensity = (1f + (-Config.Reactor.Modern.RadIntensityScalingMultiplier * Math.exp(-10f * Config.Reactor.Modern.RadIntensityScalingShiftMultiplier * Math.exp(-0.001f * Config.Reactor.Modern.RadIntensityScalingRateExponentMultiplier * (fuelHeat.temperature() - 273.15)))));
        final double fuelAbsorptionTemperatureCoefficient = (1.0 - (Config.Reactor.Modern.FuelAbsorptionScalingMultiplier * Math.exp(-10 * Config.Reactor.Modern.FuelAbsorptionScalingShiftMultiplier * Math.exp(-0.001 * Config.Reactor.Modern.FuelAbsorptionScalingRateExponentMultiplier * (fuelHeat.temperature() - 273.15)))));
